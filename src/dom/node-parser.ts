...
export const isSlotElement = (node: Node): node is HTMLSlotElement => (node as Element).tagName === 'SLOT';
// a valid custom element name must have a hyphen (but not as the first character). See https://html.spec.whatwg.org/multipage/custom-elements.html#valid-custom-element-name
export const isCustomElement = (node: Element): boolean => node.tagName.indexOf('-') > 0;
